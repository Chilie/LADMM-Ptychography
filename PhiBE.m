...
function [big_obj,aperture,fourier_error,initial_obj,initial_aperture, hist] = PhiBE(ePIE_inputs,varargin)
...
%     Probe = Probe - step_p*(P_s.*Probe - P_rhs);
    step_p = 0.9./(max(abs(P_s),2));
    Probe = Probe - step_p.*(P_s.*Probe - P_rhs);
    Probe = Probe.*P_mask;
    end
    % update the YK
    for it = 1:T_i
        Lsum = Probe.*X(Y1(it):Y2(it), X1(it):X2(it));
...
%         else
%             YK(:,:,it) = ifft2((diffpats(:,:,it)+ 0.51*abs(TTmp)).*sign(TTmp)/(1+0.51))*sqrt(size(Probe,1)*size(Probe,2));
