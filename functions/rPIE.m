...
function [big_obj,aperture,fourier_error,initial_obj,initial_aperture, hist] = rPIE(ePIE_inputs,varargin)
...
if gpu
    display('========rPIE reconstructing with GPU========')
    ...
    display('========rPIE reconstructing with CPU========')
end
...
%% Update the object
        ...
        new_rspace = buffer_rspace + conj(aperture).*diff_exit_wave ./ ...
            ((1-beta_obj).*abs(aperture).^2 + beta_obj.*probe_max);
        if do_posi == 1
...
%         drawnow 
        ...
        %figure(23); img((abs(big_obj)),['rPIE ',num2str(itt)], 'colormap', 'gray');
        figure(23); imagesc(abs(big_obj)), title(['rPIE ',num2str(itt)]); colormap gray; axis image
        hsv_ap = make_hsv(aperture,1);        
        figure(24); imagesc(hsv_ap); axis image; colormap('gray'), title(num2str(itt));
        drawnow             
