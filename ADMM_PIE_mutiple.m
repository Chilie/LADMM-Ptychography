...
function [big_obj,aperture,fourier_error,initial_obj,initial_aperture, hist] = ADMM_PIE_mutiple(ePIE_inputs,varargin)
...
hist = struct('amp',[],'angle',[], 'Rfactor',[]); %hist.Rfactor = [hist.Rfactor mean_err];
verbosity = ePIE_inputs(1).Verbosity;
...
if verbosity
   checked_dp = zeros(size(diffpats));
end
...
Lambda = zeros(size(diffpats));
Probe = initial_aperture;
PP = Probe;
XPK = initial_obj;

% generate the initilization
T_i = size(diffpats,3); % how many diffraction patterns
YK = ones(size(diffpats));
Fill_X = zeros(size(big_obj));
for it = 1:T_i
    YK(:,:,it) = XPK(Y1(it):Y2(it), X1(it):X2(it)).*Probe;
    Fill_X(Y1(it):Y2(it), X1(it):X2(it)) = ones(range(Y1(it):Y2(it))+1, range(X1(it):X2(it))+1);
end
% YK = diffpats;
P_mask = Probe~=0;
% O_mask = Fill_X ~=0;
Temp_m = zeros(size(Fill_X));
[m_s,n_s] = size(Temp_m);
m_ss = floor(m_s/3);
n_ss = floor(n_s/3);
Temp_m(m_ss:end-m_ss,n_ss:end-n_ss) = ones(m_s-2*m_ss + 1, n_s - 2*n_ss + 1);
O_mask = Temp_m ~=0;
...
%     Z2 = XPK;
    ...
    %% update the X
    RHS = zeros(size(big_obj));
    Parm = zeros(size(big_obj));
    ...
    for it = 1: T_i
        ...
        start_x = X1(it);
        end_x = X2(it);
        RHS = applyimpaint(RHS, tmp, start_y, end_y, start_x, end_x);
        Parm = applyimpaint(Parm, abs(Probe).^2,start_y, end_y, start_x, end_x);
    end
    Parm_M = Parm;
    mask_1 = Parm <= 1.0e-6;
    Parm_M(mask_1) = 1.0e-6;
    Parm_M(~mask_1) = 1.0e-6*Parm(~mask_1);
%     X = (RHS + Parm_M.*XPK)./(Parm + Parm_M);
    X = (RHS + Parm_M.*XPK)./(Parm + Parm_M);
    
    X = X.*O_mask;
    XPK = X; % X.* object_support
%     XPK = X;
    % update Probe
    if itt > 20
        P_s = zeros(size(Probe));
    P_rhs = zeros(size(Probe));
    
    for it = 1:T_i
        tmp2 = conj(X(Y1(it):Y2(it), X1(it):X2(it)));
        tmp3 = tmp2.*Z(:,:,it);
        P_rhs = P_rhs + tmp3;
        tmp4 = abs(X(Y1(it):Y2(it), X1(it):X2(it))).^2;
        P_s = P_s + tmp4;
        
    end
    step_p = 0.9/(max(abs(P_s),[],'all'));
    Probe = Probe - step_p*(P_s.*Probe - P_rhs);
...
%     Parm_s = P_s;
%     mask_2 = P_s <= 1.0e-3;
%     Parm_s(mask_2) = 1.0e-3;
%     Parm_s(~mask_2) = 1.0e+2*P_s(~mask_2);
%     
%     
%     Probe = (P_rhs + Parm_s*PP)./(P_s + Parm_s); %Parm_s
% %     PP = Probe.*P_mask; % support_mask;
% %     Probe = Probe.*P_mask;
%     PP = Probe;
    end
    % update the YK
    for it = 1:T_i
        Lsum0 = Probe.*X(Y1(it):Y2(it), X1(it):X2(it));
        if verbosity
           checked_dp(:,:,it) = fft2(Lsum0); 
        end
        Lsum = Lsum0 + Lambda(:,:,it);
%         Lsum = Probe.*X(Y1(it):Y2(it), X1(it):X2(it))+ Lambda(:,:,it);
        TTmp = fft2(Lsum)/sqrt(size(Probe,1)*size(Probe,2));
%         if itt < 200
%             YK(:,:,it) = ifft2(diffpats(:,:,it).*sign(TTmp))*sqrt(size(Probe,1)*size(Probe,2));
%         else
%             YK(:,:,it) = ifft2((diffpats(:,:,it)+ 0.01*abs(TTmp)).*sign(TTmp)/(1+0.01))*sqrt(size(Probe,1)*size(Probe,2));
%         end
%         YK(:,:,it) = ifft2((diffpats(:,:,it)+ 2.5*abs(TTmp)).*sign(TTmp)/(1+2.5))*sqrt(size(Probe,1)*size(Probe,2));
if it < 155
% YK(:,:,it) = ifft2((diffpats(:,:,it)+ 0.5*abs(TTmp)).*sign(TTmp)/(1+0.5))*sqrt(size(Probe,1)*size(Probe,2));
YK(:,:,it) = ifft2(update_mag(diffpats(:,:,it),abs(TTmp),1.1).*sign(TTmp))*sqrt(size(Probe,1)*size(Probe,2));
else
    YK(:,:,it) = ifft2((diffpats(:,:,it)+ 2.01*abs(TTmp)).*sign(TTmp)/(1+2.01))*sqrt(size(Probe,1)*size(Probe,2));
%     YK(:,:,it) = ifft2(update_mag(diffpats(:,:,it),abs(TTmp),2.01).*sign(TTmp))*sqrt(size(Probe,1)*size(Probe,2));
end

Lambda(:,:,it) = Lsum - YK(:,:,it); %0.5*abs(TTmp)).*sign(TTmp)/(1+0.5)
    end
    ...
    if verbosity
        for aper = 1:T_i
            current_dp = diffpats(:,:,aper);
            missing_data = current_dp == -1;
            check_dp = abs(checked_dp(:,:,aper));
            scaling_a = 1/sqrt(size(Probe,1)*size(Probe,2));%sum(abs(current_dp.*check_dp),'all')/norm(check_dp,'fro')^2;
            if sum_dp(aper)
                fourier_error(itt,aper) = sum(abs( current_dp(~missing_data) - scaling_a*check_dp(~missing_data)) ) ./ sum_dp(aper);
            end
        end
    end
...
%         figure(14); imagesc(hsv_ap); axis image; colormap('gray'), title(num2str(itt));
        figure(14); imagesc(abs(aperture)); axis image; colormap('gray'), title(num2str(itt));
        drawnow          
    end
...
%% Update the magnitude
    function mag_out = update_mag(measurement, z, beta)
        epsilon = 1e-6;
        iter = 5;
        mag_out = (measurement + beta*z)/(1+beta);
        for ikkk=1:iter
           mag_out =  (mag_out./sqrt(mag_out.^2+epsilon).*measurement + beta*z)/(1+beta);
        end
    end
end
